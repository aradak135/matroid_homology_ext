...
sub matroidHomology {
	my $matroidsRN = $_[0];
	my $matroidsCount = $_[1];
	my $n = $_[2];
	my $r = $_[3];
	my $rMinusOne =  int($r-1);
	my $nMinusOne = int($n-1);
	...
	return $matrixOfRangeIndicatorVectors;
...
sub main {
	...
	print "This script has not yet been enabled. \n"
	exit;
	
	# my ($n,$r,$oper,$matroidType) = getUserInfo();

	# #Obtains cursors from polyDB
	# my ($matroidsRNDomain,$matroidsCountDomain) = collectionFromPolyDB($n,$r,$matroidType);
	# my ($matroidsRNRange,$matroidsCountRange) = ($matroidsRNDomain,$matroidsCountDomain);
	
	# my $kernelMatrix;
	# my $imageMatrix;
	
	# if($oper == 1){
		# ($matroidsRNRange,$matroidsCountRange) = collectionFromPolyDB($n+1,$r+1,$matroidType);
		# $kernelMatrix = matroidHomology($matroidsRNDomain,$matroidsRNDomain,$n,$r,1,0);
		# $imageMatrix = matroidHomology($matroidsRNRange,$matroidsRNRange,int($n+1),int($r+1),1,1);
	# }
	# else{
		# ($matroidsRNRange,$matroidsCountRange) = collectionFromPolyDB($n+1,$r,$matroidType);
		# $kernelMatrix = matroidHomology($matroidsRNDomain,$matroidsRNDomain,int($n+1),$r,2,0);
		# $imageMatrix = matroidHomology($matroidsRNRange,$matroidsRNRange,int($n+1),$r,2,1);
	# }
	
	# my $latticeBasisKer = lattice_basis($kernelMatrix);
	# my $latticeBasisIm = lattice_basis($imageMatrix);
	
	# print "Dim of kernel M_{r,n}: ";
	# print rank($kernelMatrix);
	# print "\n";
	# print "Dim of image M_{r,n}->: ";
	# print rank($imageMatrix);
	# print "\n";
